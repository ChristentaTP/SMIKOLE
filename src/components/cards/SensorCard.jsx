...
export default function SensorCard({ title, value, unit, color, icon, status, statusColor, onClick }) {
  return (
    <div 
      className={`rounded-xl overflow-hidden shadow-md flex flex-col h-full ${color} ${onClick ? "cursor-pointer hover:scale-[1.02] active:scale-[0.98] transition-transform" : ""}`}
      onClick={onClick}
    >
      {/* Main Content */}
